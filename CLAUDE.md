...
- `shared/*.mjs` — single source for the tool catalog (`toolcore`), token machinery (`enginecore`), and schedules, used verbatim by both the browser and the server; each has a hand-written `.d.mts` so strict TS sees real types. Client `src/tools.ts` only adds lucide icons. `sample()` in toolcore is NOT what runs return — it only suggests output shapes for token chips.
