...
- **Projects are the tenant boundary** (`projectId` on flows, runs, connections; `data/projects.json`, "default" always exists, deletion moves contents there). Connection resolution is scoped server-side at execution time via `scopeSettings()` in queue.mjs (project-owned first, then shared); `{{config.*}}` values via `scopedGlobals()` (project's `settings.projectGlobals[pid]` over shared `settings.globals`); subflows only resolve same-project flows. The browser's active project lives in localStorage (`src/projects.ts`); `makeFlow` stamps it on every creation path. Future RBAC binds roles to these ids — see docs/PRD.md § Projects.
