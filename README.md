...
- **A real home for workflows.** The left nav has four sections — Tools, Flows, Blueprints, Config. Flows are searchable and taggable (tag chips filter the list), each shows its live status, and files move them between people and instances: Export downloads one flow as `.flow.json` (variables and tags included), Import creates a flow from a file. The Run-flow step can also pass variables into the child flow per call.
