...
All runtime settings live in the in-app Settings drawer (theme, run speed, compose model, Anthropic API key) and persist server-side in `data/settings.json` (written owner-only). With a key set, AI compose calls the Anthropic API; without one it falls back to a local keyword planner.
