...
- `server/queue.mjs` + `server/executors.mjs` — real execution: a file-backed event queue (state + not_before columns; approve() flips waiting→queued; worker loop every 250ms; schedule triggers armed via nextOccurrence; webhook runs enter via `/hooks/*`). Executors do real work (fetch, node:vm sandbox, Anthropic, spawn'd CLIs, nodemailer/pg) and throw plain-language errors when a connection is missing — never fake a success. Loop/Until/Subflow are queue-level (event context, never blocking the worker); every step emits an OTel span (run.spans, `traceAsOtlp()`, optional collector export). `pruneRuns()` caps stored runs at 40, so **Reports reads a separate persistent daily rollup** (`db.analytics[projectId][date]`, filled once per run in `finalizeRun()` via `rollUp()`, backfilled at boot, bounded ~400 days) — `getReportData()` serves all-time totals + a 30-day chart from it, not the capped run list. See docs/ARCH-QUEUE.md + docs/ARCH-AI-OTEL.md.
