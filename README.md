...
- **A real home for workflows.** A slim nav rail gives three destinations — Flows, Blueprints, Config — each a full page; the editor (palette + rail) is a mode you enter by opening a flow. Flows are searchable and taggable with live-status badges, and files move them between people and instances: Export downloads one flow as `.flow.json` (variables and tags included), Import creates a flow from a file. The Run-flow step can also pass variables into the child flow per call. The reasoning is in `docs/UX-REVIEW.md`; the rule is "a thing lives in exactly one place."
- **Blueprints.** Eighteen tagged built-in starting points (deploy, triage, forms, agents, MCP tools, polling, data sync…) plus your own — save any flow as a blueprint from the Blueprints section; custom ones persist server-side in the portable format.
