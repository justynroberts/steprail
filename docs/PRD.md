...
| Secret / connection | Strictly owned by one project. Names are unique per type *within* a project — two projects can both have a `prod-db`. |
| Config (`{{config.*}}` values) | Strictly per project (`settings.projectGlobals[projectId]`). |
...
- Flows, Reports, Secrets, and Config pages show only the active project's records; new secrets and config values belong to the active project.
